...
async function buildCodexHeader() {
  ...
  const { installed, has9RouterV2, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, has9RouterV2, settings } = result.data;
  ...
  const custom = settings?.customModels?.find(m => m.id === "custom:9RouterV2-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  const provider = settings?.models?.providers?.["9router-v2"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("9router-v2/") ? primary.replace("9router-v2/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, has9RouterV2, opencode } = result.data;
