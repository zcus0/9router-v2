...
默认 URL：
- 仪表板：`http://localhost:20135/dashboard`
...
export NEXT_PUBLIC_BASE_URLhttp://localhost:20135"
...
| `BASE_URL` |http://localhost:20135` | 云同步作业使用的服务器端内部基础 URL |
...
**云端同步错误**
- 验证 `BASE_URL` 指向您正在运行的实例（例如：`http://localhost:20135`）
...
```bash
POST httplocalhost:20135/v1/chat/completions
...
- 本地运行时（`http://127.0.0.1:20135/v1/chat/completions`）：使用 `stream=false` 和 `stream=true` 都可以工作。
