          ...
          {!checking && grokStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {configuredModel?.base_url && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    ...
                    <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                    <span className="min-w-0 truncate rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">{configuredModel.base_url} Â· {configuredModel.model}{configuredModel.context_window ? ` Â· ${(configuredModel.context_window / 1000).toLocaleString()}K ctx` : ""}</span>
                  </div>
                )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!grokStatus?.has9RouterV2} loading={restoring} className="w-full sm:w-auto"><span className="material-symbols-outlined text-[14px] mr-1">restore</span>Reset</Button>
