...
export default function JcodeToolCard({
  ...
  const currentBaseUrl = jcodeStatus?.config?.providers?.["9router-v2"]?.base_url || "";

  const getConfigStatus = () => {
    if (!jcodeStatus?.installed) return null;
    if (!jcodeStatus?.has9RouterV2) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["9router-v2"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["9router-v2"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.9router-v2]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_9ROUTER_V2_API_KEY"
env_file = "provider-9router-v2.env"
...
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    const envContent = `JCODE_9ROUTER_V2_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-9router-v2.env",
        content: envContent,
      },
    ];
  };
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["9router-v2"]?.base_url && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.has9RouterV2} loading={restoring}>
