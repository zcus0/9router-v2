...
const has9RouterV2Config = (settings) => {
  ...
  return settings.customModels.some(m => m.id?.startsWith("custom:9RouterV2"));
};
...
export async function POST(request) {
  try {
    ...
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:9RouterV2"));
