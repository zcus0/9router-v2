...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-9router-v2.env");
};
...
const has9RouterV2Config = (config) => {
  ...
  if (providers["9router-v2"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:20135")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const has9RouterV2 = has9RouterV2Config(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_9ROUTER_V2_API_KEY = apiKey;
...
export async function DELETE() {
  try {
    ...
    delete config.providers["9router-v2"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_9ROUTER_V2_API_KEY;
    await writeProviderEnv(env);
