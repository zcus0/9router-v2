...
const has9RouterV2Config = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["9router-v2"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("9router-v2");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      has9RouterV2: has9RouterV2Config(auth),
