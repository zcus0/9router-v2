...
const has9RouterV2Config = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["9router-v2"];
};

// Read per-agent models.json and return current model id (without "9router-v2/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["9router-v2"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      has9RouterV2: has9RouterV2Config(settings),
...
export async function POST(request) {
  try {
    ...
    const fullModelId = `9router-v2/${model}`;

    // Remove all old 9router-v2/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("9router-v2/"))
    ...
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`9router-v2/${m}`] = {};
    });
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("9router-v2/")) {
    ...
    settings.models.providers["9router-v2"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `9router-v2/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["9router-v2"];
    ...
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("9router-v2/"));
    ...
    if (settings.agents?.defaults?.model?.primary?.startsWith("9router-v2/")) {
