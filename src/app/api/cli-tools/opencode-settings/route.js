...
const has9RouterV2Config = (config) => {
  ...
  return !!config.provider["9router-v2"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["9router-v2"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("9router-v2/") ? config.model.replace(/^9router\//, "") : null,
...
export async function POST(request) {
  try {
    ...
    const existingProvider = config.provider["9router-v2"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["9router-v2"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `9router-v2/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `9router-v2/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("9router-v2/")) {
        config.model = "";
      }
    }
...
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["9router-v2"]?.models) {
      delete config.provider["9router-v2"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["9router-v2"].models).length === 0) {
        delete config.provider["9router-v2"];
        if (config.model?.startsWith("9router-v2/")) delete config.model;
      } else if (config.model === `9router-v2/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["9router-v2"].models);
        config.model = `9router-v2/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire 9router provider
      if (config.provider) delete config.provider["9router-v2"];
      if (config.model?.startsWith("9router-v2/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("9router-v2/")) {
